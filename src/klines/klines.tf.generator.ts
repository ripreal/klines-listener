...
@Injectable()
export class KlinesTFGenerator implements OnModuleInit {
  private readonly logger = new Logger(KlinesTFGenerator.name)
  private lastTradesIds = new Map<string, number>
  private symbols : Set<string> = new Set
  private skipEmptyCandlesSymbols : Set<string> = new Set
  ...
  async onModuleInit() {
    let symbols = await this.symbolsService.getTradeSymbols(this.getType());
  ...
  public async addTrade(trade: TradeCandle, timeframes: TimeFrameEnum[]) {
    ...
    this.symbols.add(trade.symbol)
  }

  public async generateEmptyKlines(timeframes: TimeFrameEnum[]) {
    const now = new Date
    for (let symbol of this.symbols) {
      for (let timeframe of timeframes) {
        await this.generateEmptyCandles(symbol, +now, timeframe)
      }
    }
  }
  ...
  private async generateTradeCandles(trade: TradeCandle, timeframe: TimeFrameEnum): Promise<void> {
    ...
    let foundKline = this.klinesCache.get(timeframe, trade.symbol, periodMs)

    if (!foundKline) {
      foundKline = await this.klinesService.getLastKline(timeframe, trade.symbol)
  ...
  private async generateEmptyCandles(symbol: string, time: number, timeframe: TimeFrameEnum): Promise<void> {
    ...
    let foundKline = this.klinesCache.getLast(timeframe, symbol)
    if (!foundKline) {
      if (this.skipEmptyCandlesSymbols.has(symbol)) {
        return
      }
      this.logger.verbose(`${symbol} ${timeframe} kline not found in cache. Searching in database`)
      foundKline = await this.klinesService.getLastKline(timeframe, symbol)
      this.klinesCache.put(timeframe, foundKline)
      if (!foundKline) {
        this.skipEmptyCandlesSymbols.add(symbol)
        this.logger.verbose(`${symbol} ${timeframe} kline not found in db. Skip it`)
        return
      }
    }
    
    if (timeframe == TimeFrameEnum.OneSecond) {
      await this.generateEmptyCandlesV2(symbol, time, timeframe)
      return
    }
